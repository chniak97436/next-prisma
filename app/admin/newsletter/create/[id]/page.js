...
export default function createEmailId({ params }) {
    const { id } = use(params)
    ...
    useEffect(() => {
        async function fetchNewsletter() {
            ...
            console.log("data.newsletters : ___", data.newsletter.email)
    ...
    const newsSubmit = async (e) => {
        e.preventDefault()
        const to = newsletter.email

        // Debug: Check values avant envoie
    ...
    return (
        <div className=' text-[#F5CC60]'>
            <div className="p-1 bg-[#F5CC60]/40 rounded-lg mb-10">
                ...
                <h1 className='flex text-xl font-bold my-6 w-full'><User className='w-6 h-6 inline ' />Envoyer un mail à <strong className='flex flex-row align-bottom'>  : {newsletter?.email}</strong></h1>
            </div>
